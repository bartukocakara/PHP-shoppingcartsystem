...
<?php 
    ...
    if($products_in_cart)
    {
        //There are products in the cart so we need to select thos products from the database
        //Products in cart array to question martk string array, we need the SQL statement to include IN (?, ?, ?,...etc)
        $array_to_question_marks = implode(',', array_fill(0, count($products_in_cart), '?'));
        $stmt = $pdo->prepare('SELECT * FROM products WHERE id IN ('.$array_to_question_marks . ')');
        //We only need the keys not the values, the keys are the id's of the products
        $stmt->execute(array_key_exists($products_in_cart));
